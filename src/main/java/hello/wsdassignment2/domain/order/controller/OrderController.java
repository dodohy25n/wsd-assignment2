...
public class OrderController {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "400", description = "입력값 유효성 검사 실패 또는 재고 부족",
    ...
    @Operation(summary = "주문 목록 조회 (Admin)", description = "주문 목록을 조회합니다. (관리자 권한 필요)")
    @ApiResponses(value = {
    ...
    @ApiResponses(value = {
            ...
            @ApiResponse(responseCode = "404", description = "존재하지 않는 주문",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class))),
            @ApiResponse(responseCode = "409", description = "취소할 수 없는 주문 상태 (예: 이미 배송중)",
                    content = @Content(schema = @Schema(implementation = ErrorResponse.class)))
    })
